...
contains
    ...
    subroutine Output_VTK_D(this, Results, N)
        ! Subroutine to create .vtk file
        type(Mesh), intent(inout)         :: this
        class(ResultsTypeD), intent(inout) :: Results
        class(NTypeD), intent(in)          :: N
        ...
        do jj = 1, N%Group
            ...
            write(6,12) "LOOKUP_TABLE default"
            do ii = 1, this%N_nodes
                if (Results%Flux(jj,ii) .lt. 1.0E-9_dp) then
                    Results%Flux(jj,ii) = 0.0_dp
                end if
                write(6,15) Results%Flux(jj,ii)
            end do
        end do
    ...
    end subroutine Output_VTK_D

    subroutine Output_Higher_Order_VTK_D(Properties, this, Results, N)
        ! Subroutine to create .vtk file
        type(Mesh), intent(inout)         :: this
        class(ResultsTypeD), intent(inout) :: Results
        class(NTypeD), intent(in)          :: N
    ...
    end subroutine Output_Higher_Order_VTK_D
