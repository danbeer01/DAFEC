...
! open(unit=1, file='vtk/2D/Tris.vtk', status='old', action='read')
...
! open(unit=1, file='vtk/2D/O1_quad_2.vtk', status='old', action='read')
...
! open(unit=1, file='vtk/3D/O2_Pyr_2.vtk', status='old', action='read')
...
! open(unit=1, file='inputs/Benchmarks/c5g7.txt', status='old', action='read')
...
if(Solve_Transport_Method) call print_flux(Properties, Results, N, vtk_mesh)
if(.not. Solve_Transport_Method) call print_flux_diffusion(PropertiesD, ResultsD, N_D, vtk_mesh)
